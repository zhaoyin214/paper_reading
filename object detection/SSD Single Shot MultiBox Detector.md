...
SSD（single shot detector）：将边界框（bounding boxes）的输出空间离散化为一组锚点

锚点（anchors，default boxes，default bounding boxes）：位于各特征图（feature map）的所有位置上，代表不同长宽比、不同尺度的边界框

推理阶段：在每个锚点上，SSD网络对所有类别评分（score），并对锚点形状进行调整以配匹目标形状。
...
（2）SSD网络的核心在于使用小尺寸卷积核滤波器预测一组锚点的类别评分、边界偏移（predicting category scores and box offsets for a fixed set of default bounding boxes using small convolutional filters applied to feature maps）；
（3）使用多尺度特征图、不同长宽比锚点预测；
...
SSD方法基于前向卷积网络（a feed-forward convolutional network），生成一组固定尺寸的锚点集合及锚点对应的目标类别评分，并通过非极大抑制（non-maximum suppression）生成最终检测结果。
...
预测器：$3 \times 3 \times p$小尺寸卷积核（small kernel），预测目标类别评分（a score for a category）和相对于锚点坐标的形状偏移（a shape offset relative to the default box coordinates）

#### 锚点和长宽比（default boxes and aspect ratios）

特征图上的每个给定位置有$k$个锚点，每个锚点需计算$c$个类别的评分和相对于原始锚点形状（the original default box shape）的$4$个偏移，因此每个位置上共有$(c + 4)k$个滤波器。对于尺寸为$m \times n$的特征图，共有$(c + 4)kmn$个输出。
...
在训练过程中，需要确定与真实边界框（a ground truth detection）对应的锚点，并训练网络。对于每个真实边界框，需从不同位置、不同长宽比、不同尺度的锚点中选择。

*重叠度（jaccard overlap，intersection over union，IoU）*

配匹策略：与某个真实边界框IoU大于阈值（threshold）（0.5）的所有锚点。

#### 损失函数（training objective）

匹配标识（an indicator for matching）：$x_{ij}^{p} = \{1, 0\}$，表示锚点$i$是否与类别为$p$的真实边界框$j$配匹。根据配匹策略可知，$\sum_{i} x_{ij}^{p} \geq 1$。
...
其中，$N$为匹配的锚点总数。若$N = 0$，则将$\mathcal{L}$设置为$0$。

*位置损失（localization loss，loc）*：$\mathcal{L}_{\mathrm{loc}}(x, l, g)$采用平滑$L_1$损失（smooth $L_1$ loss），回归目标为锚点的偏移，
...
其中，锚点$d$的中心点坐标为$(c_x, c_y)$、长度为$w$、宽度为$h$。
...
#### 锚点的尺度和长宽比选取（choosing scales and aspect ratios for default boxes）
...
网络中各隐含层输出特征图的感知域（receptive field）尺寸不同。SSD框架下，*锚点无需与各层实际感知域对应*。通过在给定特征图上平铺（tiling）锚点，网络能够学习到该层特征图与特定尺度目标的对应关系。

使用$m$层特征图做预测情况下，每层特征图的*锚点尺度（scale of the default boxes for each feature map）* 为：
...
在第$k$层特征图的每个位置上，所有*锚点的长宽比（aspect ratios）* 为：
...
当锚点长宽比为$1$时，额外增加一个锚点，其尺度为：

$$s_{k}^{\prime} = \sqrt{s_{k} s_{k + 1}}$$

即在特征图的每个位置上都有$6$个锚点。

*锚点中心（center of each default box）*：
...
锚点匹配后，绝大多数锚点均为负样本，当锚点数量很大时，该现象更为明显。这导致正负样本极度不平衡（a significant imbalance between the positive and negative training examples）。本文*根据最大置信损失对锚点排序*，并选取排名靠前的锚点（pick the top ones），使负、正样本（锚点）数量之比约为$3 : 1$。
...
（1）conv4_3层的尺度为$0.1$；
（2）conv4_3、conv10_2、conv11_2层输出特征图每个位置锚点个数为$4$（$\in \left\{1, 2, \frac{1}{2}\right\}$），其余各层为$6$；
...
#### 锚点形状（more default box shapes is better）
